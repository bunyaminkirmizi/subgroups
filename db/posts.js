...
async function get_post_with_user_given_vote(post_id, authenticated_user) {
	...
	try {
		...
		if (authenticated_user) {
			row["user_given_vote"] = await votes.get_vote_type(
...
async function get_post_by_group(group_id, authenticated_user, filter) {
	...
	try {
		...
		for (let index = 0; index < group_posts.length; index++) {
			...
			if (authenticated_user) {
				element["user_given_vote"] = await votes.get_vote_type(
					authenticated_user.user_id,
					element.post_id
				);
			}
...
async function get_post_upvote_count(group_id) {
	const sqltext =
		"SELECT * FROM posts WHERE group_id =$1 order by send_timestamp desc";
	const values = [group_id];
	try {
...
async function last_posts_by_user(user_id, limit) {
	const sqltext =
		"SELECT * FROM posts WHERE user_id =$1 order by send_timestamp desc limit $2";
