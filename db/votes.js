...
async function vote(user_id, post_id, type) {
	...
	const sqltext =
		"insert into vote(user_id,post_id,vote_type,vote_timestamp) values($1,$2,$3,current_timestamp)";
...
async function get_vote_count(post_id) {
	const sqltext =
		"select ((select count(*) from vote where post_id=$1 and vote_type=True) - (select count(*) from vote where post_id=$1 and vote_type=False)) as votecount;";
