...
router.post('/changebanner/:group_id', banner_upload.single('group_banner_file'), (req, res) => {
...
router.get('/delete/:group_id',auth.authentication_required,group_ownership_required, async (req, res) => {
  const group_id = req.params.group_id
...
router.get('/join/:group_id',auth.authentication_required, async (req, res) => {
  const group_id =req.params.group_id
  const user_id = req.session.user.user_id
...
router.get('/makeprivate/:group_id',auth.authentication_required,group_ownership_required, async (req, res) => {
  const group_id =req.params.group_id
  const user_id = req.session.user.user_id
...
router.get('/makepublic/:group_id',auth.authentication_required,group_ownership_required, async (req, res) => {
  const group_id =req.params.group_id
  const user_id = req.session.user.user_id
  console.log("stastus",await groups.get_status(group_id));
...
router.get('/leave/:group_id',auth.authentication_required, async (req, res) => {
		const group_id =req.params.group_id
		const user_id = req.session.user.user_id
...
router.post('/changeabout/:group_id',auth.authentication_required, async (req, res) => {
